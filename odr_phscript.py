...
ENHANCED: PH zoom (NO distance measurements)

Reverted to your version, but REMOVED:
- Luzon/Visayas/Mindanao distance lines + km labels
- Click-to-drop red dot distance measurement (and its plotting)

Kept:
- Philippines Focus zoom toggle
- Latest TIP-only window logic
- Envelope plotting (min/max + optional intermediate tracks + selected midpoint track)
...
class ReentryGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Reentry Monitor (TIP + TLE) — Latest TIP Only + Map Envelope (PH Zoom)")
    ...
    def _build_ui(self):
        ...
        self._log("Ready. Philippines zoom is enabled by default. Distance measurements removed.")
    ...
    def on_plot_envelope(self):
        try:
            ...
            # Selected central track — draw as dotted (visual only)
            ...
            # Label object near midpoint of selected track
            ...
            self._log("Envelope plotted (latest TIP only). Distances removed.")
