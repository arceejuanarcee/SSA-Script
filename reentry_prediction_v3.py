...
# KML exporters
# -----------------------------
def _split_track_at_dateline(
    lats: List[float], lons: List[float], jump_deg: float = 180.0
) -> List[List[Tuple[float, float]]]:
    """
    Split a (lat, lon) track into segments wherever the longitude jumps by
    more than jump_deg degrees — i.e. at the antimeridian — and return each
    segment as a list of (lon, lat) tuples ready for KML coords.
    """
    if not lats:
        return []
    segments: List[List[Tuple[float, float]]] = []
    cur: List[Tuple[float, float]] = [(lons[0], lats[0])]
    for i in range(1, len(lats)):
        if abs(lons[i] - lons[i - 1]) > jump_deg:
            if len(cur) > 1:
                segments.append(cur)
...
def export_kml_tracks(path: str, tracks: List[Dict[str, Any]]) -> None:
    """
    Export ground tracks to KML, splitting each track at the antimeridian
    so QGIS / Google Earth never draws the horizontal artifact lines.
    Each segment becomes a separate LineString placemark named
    "<track_name> seg N".
    """
    ...
    kml = simplekml.Kml()
    for tr in tracks:
        segments = _split_track_at_dateline(tr["lats"], tr["lons"])
        if not segments:
            continue
        if len(segments) == 1:
            ls = kml.newlinestring(name=tr["name"])
            ls.coords = segments[0]
            ls.altitudemode = simplekml.AltitudeMode.clamptoground
            ls.extrude = 0
        else:
            for seg_idx, seg_coords in enumerate(segments, 1):
                ls = kml.newlinestring(name=f"{tr['name']} seg {seg_idx}")
                ls.coords = seg_coords
                ls.altitudemode = simplekml.AltitudeMode.clamptoground
                ls.extrude = 0
    kml.save(path)
...
def export_kml_swath_polygon(path: str, center_track_lats: List[float], center_track_lons: List[float], half_width_km: float) -> None:
    """
    Export the swath as a series of MultiGeometry polygon segments, splitting
    at the antimeridian to avoid the horizontal line artifact in QGIS / Google Earth.
    Each antimeridian-safe sub-track becomes its own closed polygon.
    """
    if simplekml is None:
        raise RuntimeError("simplekml not installed. pip install simplekml")
    ...
    for lat, lon in zip(center_track_lats, center_track_lons):
        ...
        left.append((((lon - dlon + 180) % 360) - 180, lat))
        right.append((((lon + dlon + 180) % 360) - 180, lat))

    # Split the center track at dateline jumps; use the same segment boundaries
    # for left and right edges so each polygon segment is self-consistent.
    jump = 180.0
    seg_indices: List[Tuple[int, int]] = []  # (start, end) inclusive
    seg_start = 0
    for i in range(1, len(center_track_lons)):
        if abs(center_track_lons[i] - center_track_lons[i - 1]) > jump:
            seg_indices.append((seg_start, i - 1))
            seg_start = i
    seg_indices.append((seg_start, len(center_track_lons) - 1))

    kml = simplekml.Kml()
    for seg_num, (i0, i1) in enumerate(seg_indices, 1):
        if i1 <= i0:
            continue
        seg_left  = left[i0 : i1 + 1]
        seg_right = right[i0 : i1 + 1]
        poly_ring = seg_left + seg_right[::-1] + [seg_left[0]]
        pol = kml.newpolygon(name=f"Swath ±{half_width_km:.0f} km seg {seg_num}")
        pol.outerboundaryis = poly_ring
...
class ReentryGUI(tk.Tk):
    ...
    def on_run_prediction(self):
        try:
            ...
            # over the Philippine archipelago (4–22 N, 115–130 E).
            PH_LAT_MIN, PH_LAT_MAX = 4.0, 22.0
            PH_LON_MIN, PH_LON_MAX = 115.0, 130.0
            scan_step_s = 15   # 15-second resolution
            ph_crossings = []
            for t_scan in (p10, p50, p90):
                lats_s, lons_s, times_s = groundtrack_corridor(
                    self.sat, t_scan,
                    before_min, after_min, scan_step_s
                )
                for lat_s, lon_s, t_s in zip(lats_s, lons_s, times_s):
                    if (PH_LAT_MIN <= lat_s <= PH_LAT_MAX and
                            PH_LON_MIN <= lon_s <= PH_LON_MAX):
                        ph_crossings.append((t_s, lat_s, lon_s))

            if ph_crossings:
                # De-duplicate passes separated by > 10 min
                passes = []
                cur_pass = [ph_crossings[0]]
                for item in ph_crossings[1:]:
                    gap = (item[0] - cur_pass[-1][0]).total_seconds()
                    if gap > 600:
                        passes.append(cur_pass)
                        cur_pass = [item]
                    else:
                        cur_pass.append(item)
                passes.append(cur_pass)

                self._log("--- Philippines crossing(s) detected ---")
                for i, pas in enumerate(passes, 1):
                    entry_t, entry_lat, entry_lon = pas[0]
                    peak_item = max(pas, key=lambda x: x[1])  # highest latitude = deepest track
                    exit_t, exit_lat, exit_lon = pas[-1]
                    entry_utc = entry_t if isinstance(entry_t, dt.datetime) else \
                        dt.datetime.fromtimestamp(entry_t, tz=dt.timezone.utc)
                    exit_utc  = exit_t  if isinstance(exit_t,  dt.datetime) else \
                        dt.datetime.fromtimestamp(exit_t,  tz=dt.timezone.utc)
                    if entry_utc.tzinfo is None:
                        entry_utc = entry_utc.replace(tzinfo=dt.timezone.utc)
                    if exit_utc.tzinfo is None:
                        exit_utc  = exit_utc.replace(tzinfo=dt.timezone.utc)
                    self._log(
                        f"  Pass #{i}: entry {dt_to_iso_z(entry_utc)} "
                        f"(lat={entry_lat:.2f}, lon={entry_lon:.2f}) "
                        f"-> exit {dt_to_iso_z(exit_utc)} "
                        f"(lat={exit_lat:.2f}, lon={exit_lon:.2f})"
                    )
                    # Also store in pred dict for report / JSON export
                    self.pred.setdefault('ph_crossings', []).append({
                        'pass': i,
                        'entry_utc': dt_to_iso_z(entry_utc),
                        'entry_lat': round(entry_lat, 4),
                        'entry_lon': round(entry_lon, 4),
                        'exit_utc':  dt_to_iso_z(exit_utc),
                        'exit_lat':  round(exit_lat,  4),
                        'exit_lon':  round(exit_lon,  4),
                    })
            else:
    ...
    def on_export_kml(self):
        try:
            ...
            after_min  = self._get_int(self.var_after,  "After (min)")
            step_s     = self._get_int(self.var_step,   "Step (sec)")
            swath_km   = float(self._get_float(self.var_swath_km, "Swath ±km"))
            ...
            norad  = self.var_norad.get().strip()
            ts_str = dt.datetime.now().strftime("%Y%m%d_%H%M%S")

            # 1. Corridor KML (min / mid / max tracks)
            lats_min, lons_min, _ = groundtrack_corridor(self.sat, wmin, before_min, after_min, step_s)
            lats_mid, lons_mid, _ = groundtrack_corridor(self.sat, wmid, before_min, after_min, step_s)
            lats_max, lons_max, _ = groundtrack_corridor(self.sat, wmax, before_min, after_min, step_s)

            corridor_path = os.path.join(self.out_dir, f"corridor_{norad}_{ts_str}.kml")
            export_kml_tracks(
                corridor_path,
                [
                    {"name": f"Track_wmin ({dt_to_iso_z(wmin)})", "lats": lats_min, "lons": lons_min},
                    {"name": f"Track_wmid ({dt_to_iso_z(wmid)})", "lats": lats_mid, "lons": lons_mid},
                    {"name": f"Track_wmax ({dt_to_iso_z(wmax)})", "lats": lats_max, "lons": lons_max},
                ],
            )
            self._log(f"KML corridor saved: {corridor_path}")

            # 2. Swath polygon KML
            swath_path = os.path.join(self.out_dir, f"swath_{norad}_{ts_str}.kml")
            export_kml_swath_polygon(swath_path, lats_mid, lons_mid, swath_km)
            self._log(f"KML swath saved: {swath_path}")

            # 3. Impact-proxy points KML (only if prediction has been run)
            pts_path = None
            if self.pred:
                imp = self.pred.get("impact_proxy", {})
                mc  = self.pred.get("monte_carlo", {})
                pts = []
                for pct in ("p10", "p50", "p90"):
                    ...
                    if "lat" in p and "lon" in p:
                        m = p.get("meta", {})
                        pts.append({
                            "name": f"Impact {pct.upper()} ({mc.get(pct + '_utc', '')})",
                            "lat":  float(p["lat"]),
                            "lon":  float(p["lon"]),
                            "desc": (
                                f"Time UTC: {mc.get(pct + '_utc', 'N/A')}\n"
                                f"Downrange: {m.get('downrange_km', 0):.0f} km\n"
                                f"Descent time: {m.get('descent_time_s', 0):.0f} s"
                            ),
                        })
                if pts:
                    pts_path = os.path.join(self.out_dir, f"impact_points_{norad}_{ts_str}.kml")
                    export_kml_points(pts_path, pts)
                    self._log(f"KML impact points saved: {pts_path}")

            msg = f"Corridor: {corridor_path}\nSwath: {swath_path}"
            if pts_path:
                msg += f"\nPoints: {pts_path}"
            messagebox.showinfo("KML exported", msg)

        except Exception as e:
            messagebox.showerror("KML export error", str(e))
            self._log(f"ERROR: {e}")

    # ── Save raw outputs (TIP JSON, TLE, prediction JSON) ───────────────────
    def on_save_outputs(self):
        try:
            ...
            if self.tip_raw is not None:
                tip_path = os.path.join(self.out_dir, f"tip_raw_{norad}_{ts_str}.json")
                with open(tip_path, "w", encoding="utf-8") as f:
                    json.dump(self.tip_raw, f, indent=2)
                saved.append(tip_path)
                self._log(f"TIP JSON saved: {tip_path}")

            # TLE text dump
            if self.tle:
                tle_path = os.path.join(self.out_dir, f"tle_{norad}_{ts_str}.txt")
                with open(tle_path, "w", encoding="utf-8") as f:
                    f.write("\n".join(self.tle) + "\n")
                saved.append(tle_path)
                self._log(f"TLE saved: {tle_path}")

            # Prediction JSON dump
            if self.pred:
                pred_path = os.path.join(self.out_dir, f"prediction_{norad}_{ts_str}.json")
                with open(pred_path, "w", encoding="utf-8") as f:
                    json.dump(self.pred, f, indent=2, default=str)
                saved.append(pred_path)
                self._log(f"Prediction JSON saved: {pred_path}")

            if not saved:
                messagebox.showwarning("Nothing to save", "No data available to save yet.")
                return

            messagebox.showinfo("Outputs saved", "\n".join(saved))
