...
- Uses ONLY the newest TIP "batch" (same MSG_EPOCH) for window + plotting
- TIP fetch uses a limit (default 200) so we can reliably capture the newest batch
- History CSV records: latest MSG_EPOCH used + count of solutions in that batch
- Plot Envelope now always reflects only that newest MSG_EPOCH batch
...
# TIP fetch limit (how many rows we request so we can isolate the newest MSG_EPOCH batch)
...
def select_latest_tip_batch(solutions: List[TipSolution]) -> List[TipSolution]:
    """
    Keep ONLY the newest TIP "batch": all rows that share the newest MSG_EPOCH.
    This is what you want if your intent is 'latest TIP only'.
    """
    if not solutions:
        return []
    newest_msg = solutions[0].msg_epoch
    if not newest_msg:
        # fallback: take first row only
        return solutions[:1]
...
def fetch_tip(session: requests.Session, norad_id: int, tip_url_override: str = "", limit: int = DEFAULT_TIP_LIMIT) -> list:
    if tip_url_override.strip():
        ...
        # NOTE: add limit so we reliably capture the newest MSG_EPOCH batch
        url = (
...
def compute_tip_window_from_batch(solutions_latest_batch: List[TipSolution]) -> Tuple[Optional[dt.datetime], Optional[dt.datetime], List[dt.datetime]]:
    """Compute min/max decay epoch using ONLY the latest TIP batch."""
...
class ReentryGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Reentry Monitor (TIP + TLE) — Latest TIP Only + Map Envelope")
        ...
        self.solutions_latest: List[TipSolution] = []  # <-- LATEST TIP BATCH ONLY
    ...
    def _build_ui(self):
        ...
        self._log("Ready. (This version computes + plots using ONLY the newest TIP MSG_EPOCH batch.)")
    ...
    def on_fetch(self):
        try:
            ...
            # NEW: keep only newest MSG_EPOCH batch
            ...
            # Compute window ONLY from latest batch
            ...
            self._log(f"TLE loaded: {self.tle[0] if self.tle else '(none)'} | TIP fetched rows: {len(self.solutions_all)} | Latest-batch rows: {len(self.solutions_latest)}")
    ...
    def on_plot_envelope(self):
        try:
            ...
            self._log("Envelope plotted from LATEST TIP batch only (min & max bold + intermediate faint).")
    ...
    def on_save_outputs(self):
        try:
            ...
            # Plot PNG
            png_path = os.path.join(folder, f"corridor_envelope_latestTIP_{norad}_{stamp}.png")
